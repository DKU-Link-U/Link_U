  ...
  return (
    <div className="max-w-2xl mx-auto flex flex-col gap-5">
      ...
      <div className="bg-white rounded-2xl shadow-md p-6">
        <h1 className="text-base font-bold text-gray-800 mb-5">프로젝트 모집글 작성</h1>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            <label className="text-xs font-semibold text-gray-600 mb-1.5 block">프로젝트 제목 *</label>
            <input name="title" value={form.title} onChange={handleChange} required placeholder="예: 단국대 식단 알리미 앱 개발"
          ...
          <div>
            <label className="text-xs font-semibold text-gray-600 mb-1.5 block">프로젝트 설명 *</label>
            <textarea name="description" value={form.description} onChange={handleChange} required rows={4}
              placeholder="프로젝트 목표, 역할 분담, 기간 등을 작성해주세요."
          ...
          <div>
            ...
            <input name="techStack" value={form.techStack} onChange={handleChange} placeholder="예: React, Spring Boot, PostgreSQL"
              className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/10" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-semibold text-gray-600 mb-1.5 block">모집 인원</label>
              <input type="number" name="capacity" value={form.capacity} onChange={handleChange} min={2} max={20}
                className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/10" />
            </div>
            <div>
              <label className="text-xs font-semibold text-gray-600 mb-1.5 block">최소 요구 점수</label>
              <input type="number" name="requiredRating" value={form.requiredRating} onChange={handleChange} min={0} step={50}
                className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/10" />
            </div>
          </div>
          <button type="submit" className="mt-2 w-full bg-primary text-white py-3 rounded-xl font-semibold text-sm hover:opacity-90 transition-opacity">
            모집글 등록
          </button>
        </form>
      </div>
