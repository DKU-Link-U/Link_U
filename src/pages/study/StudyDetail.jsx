...
import { fetchStudyApplications, updateApplicationStatus } from '../../api'
...
export default function StudyDetail() {
  ...
  const { getStudyById, applyStudy, getStudyEligibility, updateStudyStatus } = useStudies()
  ...
  const isOwner = Boolean(sg && sg.leaderId === getUserId(currentUser))
  ...
  useEffect(() => {
    ...
    async function loadInitialApplications() {
      ...
      try {
        const list = await fetchStudyApplications(groupId, { accessToken })
  ...
  }, [accessToken, groupId, isOwner])

  const loadApplications = async () => {
    if (!sg?.groupId || !isOwner) return

    setApplicationsLoading(true)
    setApplicationsError('')

    try {
      const list = await fetchStudyApplications(sg.groupId, { accessToken })
      setApplications(list)
    } catch (error) {
      setApplicationsError(error.message || '신청자 목록을 불러오지 못했습니다.')
    } finally {
  ...
  if (!sg) {
    return (
      <div className="py-24 text-center text-gray-400">
        ...
        <Link to={ROUTE_PATHS.study.list} className="mt-2 inline-block text-xs text-primary underline">목록으로</Link>
      </div>
    )
  }
  ...
  const handleUpdateApplication = async (applicationId, status) => {
    ...
    try {
      ...
      if (status === 'accepted' && sg.capacity > 0 && 1 + acceptedCount >= sg.capacity) {
        await updateStudyStatus(sg.groupId, 'closed')
      }
    } catch (error) {
  ...
  const handleToggleRecruitmentStatus = async () => {
    ...
    try {
      await updateStudyStatus(sg.groupId, sg.status === 'recruiting' ? 'closed' : 'recruiting')
    } catch (error) {
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-5">
      ...
      <div className="rounded-2xl bg-white p-6 shadow-md">
        ...
        <h1 className="mb-3 text-xl font-bold text-gray-800">{sg.title}</h1>
        <p className="mb-5 text-sm leading-relaxed text-gray-600">{sg.description}</p>

        <div className="mb-5 flex flex-wrap gap-1.5">
          {sg.techStack.map(tech => (
            <span key={tech} className="rounded-full bg-blue-50 px-3 py-1 text-xs font-medium text-blue-600">{tech}</span>
          ))}
        </div>
        ...
        <div className="mb-5 grid grid-cols-2 gap-3 rounded-xl bg-gray-50 p-4 text-sm">
          ...
          <div>
            ...
            <p className="font-semibold text-gray-800">{displayCurrentCount}/{sg.capacity}명</p>
          </div>
          <div>
          ...
          <div className="rounded-xl bg-primary/5 px-4 py-3">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              >
                {statusUpdating
                  ? '변경 중...'
                  : sg.status === 'recruiting' ? '모집 마감' : '모집 재개'}
              </button>
            </div>
            {statusError && (
